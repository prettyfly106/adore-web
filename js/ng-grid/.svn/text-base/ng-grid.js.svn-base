...
var ngGrid = function ($scope, options, sortService, domUtilityService, $filter, $templateCache, $utils, $timeout, $parse, $http, $q) {
    ...
    // Thêm bởi NamNT
    $scope.pagerButtonClass = self.config.pagerButtonClass;
    $scope.pagerInputClass = self.config.pagerInputClass;
    //i18n support
    $scope.i18n = {};
...
window.ngGrid.i18n['vn'] = {
	    ngAggregateLabel: 'hàng',
	    ngGroupPanelDescription: 'Chọn cột để nhóm.',
	    ngSearchPlaceHolder: 'Tìm kiếm...',
	    ngMenuText: 'Chọn cột: ',
	    ngShowingItemsLabel: 'Hàng đang hiển thị: ',
	    ngTotalItemsLabel: 'Tổng số hàng:',
	    ngSelectedItemsLabel: 'Hàng đang được chọn:',
	    ngPageSizeLabel: 'Kích thước trang:',
	    ngPagerFirstTitle: 'Trang đầu',
	    ngPagerNextTitle: 'Trang kế tiếp',
	    ngPagerPrevTitle: 'Trang trước',
	    ngPagerLastTitle: 'Trang cuối'
	};
...
angular.module("ngGrid").run(["$templateCache", function($templateCache) {
  ...
  $templateCache.put("footerTemplate.html",
    ...
    "        </div>" +
    "    </div>" +
    "    <div class=\"ngPagerContainer\" style=\"float: right; margin-top: 5px;\" ng-show=\"enablePaging\" ng-class=\"{'ngNoMultiSelect': !multiSelect}\">" +
    "        <div style=\"float:left; margin-right: 10px;\" class=\"ngRowCountPicker\" ng-show=\"pagingOptions.allowChangePageSize\" >" +
    "            <span style=\"float: left; margin-top: 3px;\" class=\"ngLabel\">{{i18n.ngPageSizeLabel}} </span>" +
    "            <select style=\"float: left;height: 27px; width:auto !important\" class=\"{{pagingOptions.InputClass}}\" ng-model=\"pagingOptions.pageSize\" >" +
    "                <option ng-repeat=\"size in pagingOptions.pageSizes\">{{size}}</option>" +
    "            </select>" +
    "        </div>" +
    "        <div style=\"float:left; margin-right: 10px; line-height:25px;\" class=\"ngPagerControl\" style=\"float: left; min-width: 135px;\">" +
    "            <button class=\"glyphicon glyphicon-fast-backward {{pagingOptions.ButtonClass}}\" ng-click=\"pageToFirst()\" ng-disabled=\"cantPageBackward()\" title=\"{{i18n.ngPagerFirstTitle}}\"></button>" +
    "            <button class=\"glyphicon glyphicon-step-backward {{pagingOptions.ButtonClass}}\" ng-click=\"pageBackward()\" ng-disabled=\"cantPageBackward()\" title=\"{{i18n.ngPagerPrevTitle}}\"></button>" +
    "            <input id=\"selectPage\" class=\"{{pagingOptions.InputClass}}\" min=\"1\" max=\"{{maxPages()}}\" type=\"number\" style=\"width:50px !important;display: inline !important; height: 29px; margin-top: 1px; padding: 0 4px;\" ng-model=\"pagingOptions.currentPage\"/><label for=\"selectPage\" ng-show=\"pagingOptions.showTotalPage\">/{{maxPages()}}</label>" +
    "            <button class=\"glyphicon glyphicon-step-forward {{pagingOptions.ButtonClass}}\" ng-click=\"pageForward()\" ng-disabled=\"cantPageForward()\" title=\"{{i18n.ngPagerNextTitle}}\"></button>" +
    "            <button class=\"glyphicon glyphicon-fast-forward {{pagingOptions.ButtonClass}}\" ng-click=\"pageToLast()\" ng-disabled=\"cantPageToLast()\" title=\"{{i18n.ngPagerLastTitle}}\"></button>" +
    "        </div>" +
    "    </div>" +
    "</div>"
  );
